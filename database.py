...
class Database:
    ...
    def check_if_link_exist(self, added_values: list, table='items'):
        query = f"""SELECT t.link FROM {table} t WHERE t.link in {tuple(added_values)}"""
        self.cursor.execute(query)
    ...
    def replace_estimated_cards(self, column_values: pd.DataFrame, table: str = 'items'):
        columns = ', '.join(column_values.columns)
        values = [tuple(value) for value in column_values.values]
        self.cursor.executemany(
            f"REPLACE INTO {table} ({columns}) VALUES({','.join('?' * len(column_values.columns))}", values
        )
        self.conn.commit()
